...
const carSales = document.querySelector(".carSales");

salesByWeek.forEach((sale) => {
  carSales.innerHTML += `<h2>${sale.sales_agent.first_name} ${sale.sales_agent.last_name}`;
  for (const car of Object.entries(sale.vehicle)) {
    const capitalizedKey = car[0][0].toUpperCase() + car[0].slice(1);
    carSales.innerHTML += `<div>${capitalizedKey}: ${car[1]}`;
  }
  carSales.innerHTML += `<h3>Profit: $${sale.gross_profit}</h3><hr />`;
});
